#!/usr/bin/env bash
# bootstrap.sh — initialize a fresh Mac Mini with vectorious/linkvectorized config
# Run with: bash <(curl -fsSL https://raw.githubusercontent.com/linkvectorized/dotfiles/main/bootstrap.sh)
# Or after cloning: bash ~/dotfiles/bootstrap.sh
...
# ── 7. Clone trading-agent-system ────────────────────────────────────────────
TRADING_DIR="$HOME/trading-agent-system"
if [ ! -d "$TRADING_DIR/.git" ]; then
  echo "==> Cloning trading-agent-system..."
  gh repo clone linkvectorized/trading-agent-system "$TRADING_DIR"
else
  echo "==> trading-agent-system already cloned"
fi

# ── Done ──────────────────────────────────────────────────────────────────────
echo ""
echo "==> Bootstrap complete! Restart your terminal (or run: source ~/.bash_profile)"
echo ""
echo "    Installed:"
...
echo "      2. Start trading agent: cd ~/trading-agent-system"
